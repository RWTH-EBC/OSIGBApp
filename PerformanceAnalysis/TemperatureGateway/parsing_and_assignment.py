...
#%%
def add_topics(d_all, topic, object_id):
    try:
        d_all['topic'] = topic
        d_all['object_id'] = object_id
    except:
        print('Could not find data point in data points, continuing with next entry.')
    return d_all
...
def parseJSONpublishFIWARE(key, value):
    res = json.dumps({key: value}, separators=(',', ':'))
    return res
