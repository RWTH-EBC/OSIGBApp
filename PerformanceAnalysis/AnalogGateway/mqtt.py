...
#%%    
    ...
    def connect(self, client=None, host=None, port=None, keepalive=None, clientID=None):
        ...
        if client:
            ...
            if clientID:
                ...
                self.client = PahoMQTTClient(client_id=f"{self.mqtt_username}-{clienttime}", protocol=MQTTv31, clean_session=False)
        ...
        # Subscribe topics if available
        if self.subscriptions:
            for i in self.subscriptions:
                self.client.subscribe(i, qos=0)
    ...
    def on_disconnect(self, client=None, userdata=None, rc=None):
        print('Disconnect?')
        if rc != 0:
            print("Unexpected MQTT disconnection. Try reconnecting..")
            self.connect()
...
#%%
    def start_mqtt(self):
        ...
        # self.client.loop_forever() # blocks main thread!!!!
