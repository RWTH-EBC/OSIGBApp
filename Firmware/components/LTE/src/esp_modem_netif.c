...
/**
 * @brief User init default station (un-official API)
 */
esp_netif_t* esp_netif_create_default_ppp(modem_dte_t *dte)
{
    esp_netif_config_t cfg = ESP_NETIF_DEFAULT_PPP();
    esp_netif_t *netif = esp_netif_new(&cfg);
    assert(netif);
    esp_netif_ppp_set_auth(netif, NETIF_PPP_AUTHTYPE_PAP, CONFIG_ISP_PPP_AUTH_USERNAME, CONFIG_ISP_PPP_AUTH_PASSWORD);
    void *modem_netif_adapter = esp_modem_netif_setup(dte);
    esp_netif_attach(netif, modem_netif_adapter);
