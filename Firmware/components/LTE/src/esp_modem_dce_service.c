...
{
    ESP_LOGI(DCE_TAG, "Echo off [ATE0]");
    modem_dte_t *dte = dce->dte;
    dce->handle_line = esp_modem_dce_handle_response_default;
    if (on) {
...
{
    ESP_LOGI(DCE_TAG, "Store profile [AT&W]");
    modem_dte_t *dte = dce->dte;
    dce->handle_line = esp_modem_dce_handle_response_default;
...
{
    ESP_LOGI(DCE_TAG, "Set flow control [AT+IFC]");
    modem_dte_t *dte = dce->dte;
    char command[16];
...
{
    ESP_LOGI(DCE_TAG, "Define PDP context [AT+CGDCONT]");
    modem_dte_t *dte = dce->dte;
    char command[64];
